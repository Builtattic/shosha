...
type ConnectionUser = {
  ...
  isSelf: boolean;
  isFollowing: boolean;
  /** Followed via swipe / followingAccounts (account dossier), not user-to-user graph. */
  isAccountDossier?: boolean;
};
...
type ConnectionListModalProps = {
  targetUserId: string;
  followersCount: number;
  followingCount: number;
  className?: string;
  /** When false, only the overlay list is used; open via ref. Default true. */
                  ...
                  <div className="flex flex-col items-center gap-2 py-10 text-center">
                    ...
                    <p className="text-[12px] text-muted-foreground">
                      {activeType === 'following'
                        ? 'Follow people or right-swipe profiles to see them here.'
                  ...
                  <div className="space-y-2">
                    {users.map((item) => (
                      <div key={item._id} className="flex items-center gap-3 rounded-xl border border-border p-3">
                        <Link
                          href={
                            item.isAccountDossier
                              ? `/account/${encodeURIComponent(item._id)}`
                              : `/account/website_${item.username.replace(/^@/, '')}`
                          }
