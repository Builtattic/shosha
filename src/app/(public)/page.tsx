...
export default function HomePage() {
  return (
    <main className="w-full relative bg-white min-h-screen font-sans selection:bg-primary selection:text-white">
      {/* HEADER */}
      <header className="absolute top-0 left-0 right-0 z-50 flex items-center justify-between px-6 py-6 md:px-12">
        ...
        <div className="flex gap-4">
          <Link href="/dashboard" className="rounded-full bg-foreground px-6 py-2.5 text-[14px] font-bold text-white transition-all hover:bg-foreground/90 hover:scale-105 active:scale-95">
            Launch App
          </Link>
        </div>
      </header>
      ...
      <section className="relative flex flex-col items-center justify-center pt-40 pb-20 px-6 text-center md:pt-56 md:pb-32">
        ...
        <div className="relative z-10 mt-12 flex flex-col gap-4 animate-fadeUp opacity-0 sm:flex-row" style={{ animationDelay: '0.4s' }}>
          <Link href="/dashboard" className="flex items-center justify-center gap-2 rounded-full bg-primary px-8 py-4 text-[15px] font-bold text-white shadow-lg shadow-primary/20 transition-all hover:bg-primary/90 hover:scale-105 active:scale-95">
            Explore the Feed <ArrowRight size={18} />
          </Link>
          <Link href="/ranks" className="flex items-center justify-center gap-2 rounded-full border border-border bg-white px-8 py-4 text-[15px] font-bold text-foreground transition-all hover:bg-muted active:scale-95">
            View Global Ranks
          </Link>
        </div>
      </section>
      ...
      {/* FOOTER */}
      <footer className="bg-white py-12 px-6 text-center border-t border-border">
