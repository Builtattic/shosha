...
export async function GET() {
  try {
    ...
    console.error('[accounts api] GET failure:', error);
    return fail('internal_error', 'Could not retrieve dossiers.', 500);
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const effectiveSource = parsed.data.sourceUrl ?? (parsed.data as any).socialUrl;
    ...
    let account = await accountsRepo.create({
      ...
      displayName,
      bio: profile.bio || '',
      avatarUrl: profile.avatarUrl || `https://api.dicebear.com/9.x/initials/svg?seed=${encodeURIComponent(displayName)}`,
    ...
    const extras = (parsed.data as any).additionalSocialLinks;
    if (extras?.length) {
    ...
    console.error('[accounts api] POST failure:', error);
    return fail('internal_error', error instanceof Error ? error.message : 'Could not open target dossier.', 500);
  }
}
