...
// ── Platform icon ─────────────────────────────────────────────────────────────
// lucide-react doesn't ship brand icons — use compact text badges instead.
...
function PlatformIcon({ platform }: { platform?: string }) {
  ...
  const label = platform ? (map[platform.toLowerCase()] ?? null) : null;
  if (label) {
    return (
...
export function FeedItem({
  ...
  const [viewerState, setViewerState] = useState(viewer ?? { vote: null as 'align' | 'oppose' | null, bookmarked: false });
  ...
  async function handleShareClick(e?: React.MouseEvent) {
    ...
    try {
      ...
      // @ts-ignore — html2canvas has no bundled types; install @types/html2canvas if needed
      ...
      toast.push('Could not generate image. Copying link instead.');
    ...
    try {
      ...
      if (navigator.share && navigator.canShare?.({ files: [file] })) {
        await navigator.share({
          title: `Report on Shosha: ${user.name || 'Someone'}`,
          text: `Report on Shosha: ${user.name || 'Someone'} · ${deed || category || title || 'a report'}`,
          url: `${siteUrl()}/post/${id}`,
        });
        return;
      }
  ...
  async function interact(
    action: 'align' | 'oppose' | 'share' | 'bookmark',
    e?: React.MouseEvent,
  ) {
    e?.stopPropagation();
    setBusy(action);
  ...
  const isLiveNews =
    id.startsWith('twitter-') || id.startsWith('ig-') ||
    id.startsWith('fb-') || id.startsWith('news-') || id.startsWith('reddit-');
  ...
  const subjectLink = displaySubject
    ? `/account/${(displaySubject as typeof user).accountId || displaySubject.handle}`
    : null;
    ...
    >
      ...
      <div className="flex flex-col gap-3 p-5 pb-3">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
              ...
              <div
                className={cn(
                  ...
                  displayAuthor.avatar &&
                    displayAuthor.avatar !== 'null' &&
                    displayAuthor.avatar !== 'undefined'
                    ? 'hidden'
                    : undefined,
                )}
              >
            ...
            <div className="flex flex-col">
              <div className="flex items-center gap-1">
                <Link
                  to={authorLink}
                  className="text-[16px] font-bold text-foreground leading-tight hover:underline transition-all"
                >
                  {displayAuthor.name}
                </Link>
                {displayAuthor.isVerified && (
                  <CheckCircle2 size={16} className="text-foreground fill-foreground/10" />
                )}
              </div>
          ...
          <motion.div
            ...
            className={cn(
              'inline-flex h-7 min-h-7 items-center justify-center gap-1.5 rounded-full px-3 py-0 text-[11px] font-black uppercase tracking-[0.12em] transition-all',
              isPositive
      ...
      <div className="px-5 py-4">
        {(category || deed || reportScore !== undefined || disputeStatus) && (
          <div className="mb-3 flex flex-wrap gap-2">
            {category && (
              <span className="inline-flex h-6 min-h-6 items-center justify-center rounded-full border border-border bg-muted px-2.5 py-0 text-[10px] font-bold uppercase tracking-wider text-muted-foreground leading-none align-middle">
                {category}
              </span>
            )}
            {evidenceSourceUrl && (
              <a
                ...
                className="inline-flex h-6 min-h-6 items-center justify-center rounded-full border border-primary/20 bg-primary/5 px-2.5 py-0 text-[10px] font-bold uppercase tracking-wider text-primary hover:bg-primary/10 transition-colors leading-none align-middle"
              >
                Source
              </a>
            )}
            {deed && (
              <span className="inline-flex h-6 min-h-6 items-center justify-center rounded-full border border-primary/20 bg-primary/10 px-2.5 py-0 text-[10px] font-bold uppercase tracking-wider text-primary leading-none align-middle">
                {deed}
              </span>
            )}
            {reportScore !== undefined && (
              <span className="inline-flex h-6 min-h-6 items-center justify-center rounded-full border border-border bg-background px-2.5 py-0 text-[10px] font-bold uppercase tracking-wider text-foreground leading-none align-middle">
      ...
      <div className="flex items-center justify-between border-t border-border/60 bg-muted/20 px-5 py-3 text-muted-foreground">
        ...
        <button
          ...
          className={cn(
            'transition-colors hover:text-foreground active:scale-95 disabled:opacity-60',
            viewerState.bookmarked && 'text-foreground',
          )}
        >
                          ...
                          <Link
                            to={
                              c.author.username === 'anonymous'
                                ? '#'
                                : `/account/website_${c.author.username.replace(/^@/, '')}`
                            }
                            className={cn(
          ...
          <div className="fixed inset-0 z-[110] flex items-end justify-center bg-background/90 p-4 backdrop-blur-sm sm:items-center">
            <div className="w-full max-w-md rounded-2xl border border-border bg-card p-5 shadow-2xl">
              ...
              <input
                ...
                onChange={(e) => setModerationEvidenceUrl(e.target.value)}
