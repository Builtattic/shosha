...
export function MobileMenu({ open, onClose }: MobileMenuProps) {
  ...
  const displayName = me?.name || firebaseUser?.displayName || firebaseUser?.email?.split('@')[0] || 'Welcome';
  const username = me?.username || (firebaseUser?.email?.split('@')[0] ?? '');
  const rawAvatar = me?.photoUrl || firebaseUser?.photoURL;
