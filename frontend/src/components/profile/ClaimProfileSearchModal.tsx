...
export function ClaimProfileSearchModal({
  ...
  // Debounced search using API proxy
  useEffect(() => {
    if (!open) return;
    const q = query.trim();
    if (q.length < 2) {
        ...
        >
          ...
          >
            ...
            <div className="flex-1 -mx-1 overflow-y-auto px-1 pb-1">
              {error && (
                <div className="flex items-start gap-2 rounded-xl border border-destructive/30 bg-destructive/10 p-3 text-[12px] font-medium text-destructive">
