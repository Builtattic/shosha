    ...
    <main className="w-full relative min-h-screen bg-background font-sans selection:bg-primary selection:text-primary-foreground">
      ...
      <header className="absolute top-0 left-0 right-0 z-50 flex items-center justify-between px-6 py-6 md:px-12">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            to="/dashboard"
            className="rounded-full bg-foreground px-6 py-2.5 text-[14px] font-bold text-background transition-all hover:bg-foreground/90 hover:scale-105 active:scale-95"
          >
            Launch App
          </Link>
        </div>
      ...
      <section className="relative flex flex-col items-center justify-center pt-40 pb-20 px-6 text-center md:pt-56 md:pb-32">
        ...
        {/* Pill badge */}
        <div
          className="relative z-10 animate-fadeUp opacity-0"
        ...
        {/* Headline */}
        <h1
          className="relative z-10 mt-8 max-w-[900px] animate-fadeUp opacity-0 font-serif text-[clamp(48px,8vw,100px)] font-black leading-[0.95] tracking-[-0.04em] text-foreground"
        ...
        {/* Sub-headline */}
        <p
          className="relative z-10 mt-8 max-w-[540px] animate-fadeUp opacity-0 text-[16px] leading-[1.6] text-muted-foreground md:text-[18px]"
        ...
        {/* CTAs */}
        <div
          className="relative z-10 mt-12 flex flex-col gap-4 animate-fadeUp opacity-0 sm:flex-row"
          style={{ animationDelay: '0.4s' }}
        >
          <Link
            to="/dashboard"
            className="flex items-center justify-center gap-2 rounded-full bg-primary px-8 py-4 text-[15px] font-bold text-primary-foreground shadow-lg shadow-primary/20 transition-all hover:bg-primary/90 hover:scale-105 active:scale-95"
          >
            Explore the Feed <ArrowRight size={18} />
          </Link>
          <Link
            to="/ranks"
            className="flex items-center justify-center gap-2 rounded-full border border-border bg-background px-8 py-4 text-[15px] font-bold text-foreground transition-all hover:bg-muted active:scale-95"
          >
            View Global Ranks
          </Link>
        </div>
