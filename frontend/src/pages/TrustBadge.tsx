...
const BENEFITS = [
  {
    ...
    description: 'Your profile credibility jumps to 100% — the maximum possible.',
  },
  {
    Icon: Eye,
    title: 'More Visibility',
    description: 'Verified profiles rank higher in discovery, people deck, and search.',
  },
  {
    Icon: FileCheck,
    title: 'Stronger Reports',
    description: 'Your filings carry higher trust weight in AI and community adjudication.',
  },
  {
    Icon: ShieldCheck,
    title: 'Identity Protection',
    description: 'Reduces impersonation risk and confirms you own your presence.',
  },
] as const;
...
const STEPS = [
  ...
  },
  {
    ...
    description: `A monthly fee of $${TRUST_BADGE_USD} / ₹${TRUST_BADGE_INR} covers the cost of identity review. $1/mo or ₹99/mo · Cancel anytime`,
  },
  {
    number: '03',
  ...
  useEffect(() => {
    ...
    apiClient
      ...
      .then((res) => {
        ...
        } else if (u.trust_badge_pending) {
          setVerificationState('pending');
        } else if (u.trust_badge_rejected_at) {
          setVerificationState('rejected');
          setRejectionReason(u.trust_badge_rejection_reason ?? null);
        } else {
    ...
    <main className="min-h-screen bg-background safe-bottom">
      ...
      <div className="mx-auto max-w-[860px] px-7">
        ...
        <div className="py-16 border-b border-border">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="rounded-[24px] border border-border bg-card p-8">
              ...
              <h3 className="font-serif text-[28px] font-normal text-foreground mb-6">What it is</h3>
              <ul className="space-y-3">
                {WHAT_IT_IS.map((item) => (
                  <li key={item} className="text-[16px] text-foreground leading-relaxed">{item}</li>
                ))}
              </ul>
            </div>
            <div className="rounded-[24px] border border-border bg-muted p-8">
              <Shield size={32} className="mb-4 text-muted-foreground" />
              <h3 className="font-serif text-[28px] font-normal text-muted-foreground mb-6">What it isn&apos;t</h3>
              <ul className="space-y-3">
                {WHAT_IT_ISNT.map((item) => (
                  <li key={item} className="text-[16px] text-muted-foreground leading-relaxed">{item}</li>
                ))}
              </ul>
            </div>
          </div>
        ...
        <div className="py-16 border-b border-border">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {BENEFITS.map(({ Icon, title, description }) => (
              <div key={title} className="rounded-[24px] border border-border bg-card p-6 flex flex-col gap-3">
          ...
          <p className="text-[13px] text-muted-foreground mt-4">
            Credibility score increases by up to 20 points upon verification approval.
            Maximum unverified credibility is 80%.
          </p>
        </div>
        ...
        <div className="py-16 border-b border-border">
          ...
          <div>
            {STEPS.map((step) => (
              <div key={step.number} className="flex gap-6 py-6 border-b border-border last:border-0">
                <span className="font-mono text-[12px] text-muted-foreground shrink-0 pt-1">{step.number}</span>
                <div>
                  <h3 className="font-serif text-[24px] font-normal text-foreground">{step.title}</h3>
                  <p className="text-[16px] text-muted-foreground leading-relaxed mt-2">{step.description}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="py-20 flex flex-col items-center text-center gap-6">
          ...
          {verificationState === 'unauthenticated' && (
            <>
              ...
              <p className="text-[18px] text-muted-foreground max-w-[480px]">
                Sign in to apply for the Trust Badge and unlock verified status on Shosha.
              </p>
              <button
                type="button"
          ...
          {verificationState === 'unverified' && (
            <>
              ...
              <p className="text-[18px] text-muted-foreground max-w-[480px]">
                Join verified contributors on Shosha™. One-time verification. Permanent badge.
              </p>
              <button
                type="button"
          ...
          {verificationState === 'pending' && (
            <>
              ...
              <p className="text-[18px] text-muted-foreground max-w-[480px]">
                Your identity is currently under review. This typically takes 2-5 business days.
              </p>
              ...
              <Link to="/profile" className="text-[12px] text-muted-foreground underline underline-offset-2 hover:text-foreground">
                Back to your profile
              </Link>
            </>
          )}
          ...
          {verificationState === 'verified' && (
            <>
              ...
              <p className="text-[18px] text-muted-foreground max-w-[480px]">
                Your Trust Badge is active. Your credibility score reflects your verified status.
              </p>
